...
export class EscrowModel {
    static async payEscrow(ctx: Context, input: z.infer<typeof payEscrowInputSchema>) {
        ...
        return {
            ...
            escrow: {
                ...
                status: escrow.status as "HOLDING" | "RELEASED" | "REFUNDED",
            },
            landStatus: application.land.status as "AVAILABLE" | "IN_NEGOTIATION" | "LEASED" | "HIDDEN",
        };
    }

    static async verifyMalpotPapers(ctx: Context, input: z.infer<typeof verifyMalpotPapersInputSchema>) {
        const application = await ctx.prisma.application.findUnique({
            where: { id: input.applicationId },
        ...
        return {
            ...
            application: { id: results[1].id, status: results[1].status as "PENDING" | "ACCEPTED" | "REJECTED" | "COMPLETED" },
            landStatus: results[2].status as "AVAILABLE" | "IN_NEGOTIATION" | "LEASED" | "HIDDEN",
            escrowStatus: results[3].status as "HOLDING" | "RELEASED" | "REFUNDED",
        };
    }
}
